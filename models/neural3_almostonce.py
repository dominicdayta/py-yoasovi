...
from src.almostonce import run_almostonce
from src.evaluation import compute_rmse, compute_dic
from models.neural3 import BayesianNeuralNetwork3Layer

def main():
    parser = argparse.ArgumentParser(description="YOASOVI Sample Size Analysis (YOA-AO)")
    ...
    output_dir = os.path.join(os.path.dirname(__file__), '..', 'results', 'neural3_almostonce')
    ...
    methods = {
        ...
        "LowRank": run_lowrank,
        "QMCVI_10": run_qmcvi,
        "YOASOVI_1": lambda m, *a, **kw: run_almostonce(m, *a, num_particles=1, M_init=0.1, M_max=15.0, **kw),
        "YOASOVI_3": lambda m, *a, **kw: run_almostonce(m, *a, num_particles=3, M_init=0.1, M_max=15.0, **kw),
        "YOASOVI_5": lambda m, *a, **kw: run_almostonce(m, *a, num_particles=5, M_init=0.1, M_max=15.0, **kw),
        "YOASOVI_10": lambda m, *a, **kw: run_almostonce(m, *a, num_particles=10, M_init=0.1, M_max=15.0, **kw)
    }
    ...
    for method_name, method_func in methods.items():
        print(f"\n{'='*50}")
        print(f"Executing Method: {method_name} across {n_trials} trials...")
        print(f"{'='*50}")
        ...
        filename = os.path.join(output_dir, f"{method_name}_{dataset_name}_results.pkl")
